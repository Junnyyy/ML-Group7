...
for (i in 1:10) {
  ...
  if(rows < 2) {
    ...
    test.sensitivity = 0/conf.test[1,2]
    test.specificity = conf.test[1,1]/conf.test[1,1]
  } else {
    test.error[i] = (conf.test[1,2]+conf.test[2,1])/sum(conf.test)
    test.sensitivity = conf.test[2,2]/conf.test[1,2]+conf.test[2,2]
    test.specificity = conf.test[1,1]/conf.test[1,1]+conf.test[2,1]
  }
}
#Train error rate for each sample:
train.error
#mean of the train error rate
train.error.mean = mean(train.error)
train.error.mean

#Test error rate for each sample:
test.error
